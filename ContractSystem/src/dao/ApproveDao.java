...
public class ApproveDao {
	public static  ArrayList<Contract> sped(int use_id) throws SQLException{//返回已经会签的列表
		...
		String sql="select con_id from t_contract_process where state!=1 and type=2 and user_id="+use_id;
	...
	public static  ArrayList<Contract> spContract(int use_id) throws SQLException{//返回需要审批的
		...
		String sql="select t_contract_process.con_id from t_contract_process,t_contract_state where t_contract_process.con_id=t_contract_state.con_id and t_contract_state.type=4 and t_contract_process.state=1 and t_contract_process.type=2 and user_id="+use_id;
