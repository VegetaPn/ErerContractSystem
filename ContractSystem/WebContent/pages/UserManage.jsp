...
<body>
    ...
    <div id="wrapper">
        ...
        <%String apath = this.getServletContext().getRealPath("");
          ...
          LogDao.addaLog("Enter User-Manage page", apath, uname);
        %>
        <form method="post" action="../UserProfileServlet">
                        ...
                        <div class="panel-body">
                            ...
                            <button type="button" class="btn btn-default" onclick="window.location.href='../regesiter.jsp'">添加新用户</button>
                        </div>
                        </form>
                        <form method="post" action="../PermissionServlet" id="perm">
                        <div class="panel-body">
                            <div class="dataTable_wrapper">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
                                    <%@page import="java.util.*,java.sql.*,service.RoleService"%>
                                    ...
                                    <%UserService us = new UserService();
                                      ...
                                      while(ids.next()) {
                                    	  for(int i = 1; i <= col; i++) {
                                    		  ...
                                    		  ResultSet rns = rs.getallRoleNames();
                                    		  out.print(
                                    				  "<tr class='info'>"+
	                                                      "<td>"+thisid[i]+"</td>"+
                                    		  ...
                                    		  while(rns.next()) {
                                    			  String thisroleName = rns.getString(1);
                                    			  if(role.getName().equals(thisroleName)) {
                                    				  select1 = "selected='selected'";
                                    			  } else {
                                    				  select1 = "";
                                    			  }
                                    			  out.print("<option value ='"+thisroleName+"'"+
                                    			  				select1+">"+thisroleName+"</opinion>");
                                    		  }
                                    		  idnum = "id"+n;
                                              out.print(              		  
	                                                      "</select></td>"+
	                                                      "<td>"+role.getDescription()+"</td>"+
	                                                      "<td><input name='"+idnum+"' type='checkbox' value='"+thisid[i]+"'></td>"+
                                                  	  "</tr>");
                            ...
                            <!-- /.table-responsive -->
                            <button type="submit" class="btn btn-default" style='float:right'>保存方案&删除已选用户</button>
                        </div>
                        <%
                        //if(dis.equals("operator")) {
                        //	out.println("<button type='button' class='btn btn-default'>您没有权限保存此项设置</button>");
                        //} else {
                        //	out.println("<button type='submit' class='btn btn-default'>保存角色分配方案</button>");
                        //} %>
