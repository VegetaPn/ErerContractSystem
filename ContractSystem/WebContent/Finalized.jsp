     ...
     <!-- Page Content -->
        <div id="page-wrapper">
            ...
            <div class="dataTable_wrapper">
                        ...
                        <td class="center">
                        <button type="button" class="btn btn-info" onclick="window.location.href='FinalizedServlet?content=<%=((Contract)contracts.get(i)).getContent()%>&time=<%=((Contract)contracts.get(i)).getBeignTime()%>&type=detail&id=<%=((Contract)contracts.get(i)).getId()%>&conname=<%=((Contract)contracts.get(i)).getName()%>'" id="allocate<%=i %>">定稿</button> 
                        <button  class="btn btn-info" data-toggle="modal" data-target="#myModa<%=i %>"  >查看会签意见</button>
                            ...
                            <div class="modal fade" id="myModa<%=i %>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                                            <h4 class="modal-title" id="myModalLabel">查看会签意见</h4>
                                        </div>
                                        <div class="modal-body">
                                        
                                        <%  ArrayList<String> hqcontent=FinalizedDao.seehqContent(((Contract)contracts.get(i)).getId());%>
                                        <%for(int a=0;a<hqcontent.size();a++) {%>
                                            <p><%=hqcontent.get(a)%></p>
                                          <%} %>
